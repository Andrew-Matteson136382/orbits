...
R1 = squeeze(R1);
R2 = squeeze(R2);
ta = acos(dot(R1,R2)/(norm(R1,"fro")*norm(R2,"fro")));
if R1(1)*R2(2)-R1(2)*R2(1) < 0
    ta = 2*pi - ta;
end

% Find Chord
c = sqrt(norm(R1,"fro")^2 + norm(R2,"fro")^2 - 2*norm(R1,"fro")*norm(R2,"fro")*cos(ta));

% Find Semi-Perimeter
s = (norm(R1,"fro") + norm(R2,"fro") + c)/2;

% Find Lambda
lambda = sqrt(norm(R1,"fro")*norm(R2,"fro"))*cos(ta/2)/s;

% Find w
w = atan((norm(R2,"fro")/norm(R1,"fro"))^0.25)-(pi/4);
...
P0 = c^2*(1+x)^2/(16*A*x);
P  = 4*norm(R1,"fro")*norm(R2,"fro")*P0*sin(ta/2)^2/c^2;
F  = 1-(norm(R2,"fro")/P)*(1-cos(ta));
G  = norm(R1,"fro")*norm(R2,"fro")*sin(ta)/sqrt(mu*P);
FDOT = sqrt(mu/P)*tan(ta/2)*((1-cos(ta))/P-1/norm(R2,"fro")-1/norm(R1,"fro"));
GDOT = 1-(norm(R1,"fro")/P)*(1-cos(ta));
for K=1:3
